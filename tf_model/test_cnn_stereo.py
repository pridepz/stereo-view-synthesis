...
'''
   ...
   forward of SPN:
        set W1,W2,W3 to ones

        need to code the trainning part
'''
...
def disp_net_dapangzi6_deploy(_L, _R, _D_aug, _weights, _biases):
...
#            tf.summary.image('conv0_L', conv0_L)
    ...
    with tf.variable_scope('hou'):
        ...
        with tf.variable_scope('predict_flow1'):
            ...
            print('predict flow1 shape:'+str(predict_flow1.get_shape()))

        with tf.variable_scope('loss_total'):
            loss_all = loss1 #0.2 * loss6 + 0.6 * loss4 + loss2 + loss1
    return predict_flow1_relu, loss_all  #, loss6
...
with graph.as_default():
    ...
    predict_flow1, loss_disp = disp_net_dapangzi6_deploy(L, R, D, weights, bias)
...
#    sess = tf_debug.LocalCLIDebugWrapperSession(sess)
    ...
    while step < training_iters:
        if (step == 2 or step == 4 or step == 6 ) or (step % display_step == 0):
            ...
            print("flow1:")
            print flow1[0,100:102,100:102,0]
            print("conv0:")
            print(conv0_[0:3,0:3,0,0])
